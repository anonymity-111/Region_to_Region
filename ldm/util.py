...
def replace_layers(model, old, new):
    for n, module in model.named_children():
        ...
        if isinstance(module, old):
            ## simple module
            setattr(model, n, new(in_channels=getattr(model, n).in_channels,
                                  out_channels=getattr(model, n).out_channels,
                                  temb_channels=getattr(model, n).temb_channels,
                                  dropout=getattr(model, n).dropout_p))
