...
def blend_c(target: Tensor, source: Tensor, mask: Tensor, corner_coord: Tensor, mix_gradients: bool,
               channels_dim: Optional[int] = None, green_function: Optional[Tensor] = None, bbox: tuple = None,
               integration_mode: str = 'origin') -> Tensor:
    # Zero edges of mask, to avoid artefacts
    for d in range(len(mask.shape)):
        mask[tuple([[0, -1] if i == d else slice(None) for i in range(len(mask.shape))])] = 0
    ...
    chosen_dimensions = [d for d in range(num_dims) if d != channels_dim]
    corner_dict = dict(zip(chosen_dimensions, corner_coord.numpy()))
    # corner_dict = {dim: corner_coord[dim].item() for dim in chosen_dimensions}
    ...
    indices_to_blend1 = [slice(corner_dict[i], corner_dict[i] + s_s) if i in chosen_dimensions else slice(None)
                        for i, s_s in enumerate([3,ylen,xlen])]

    # new_source = torch.zeros_like(target)
    # new_source[tuple(indices_to_blend)] = source

    new_source1 = torch.zeros_like(target)
    new_source1[tuple(indices_to_blend1)] = source[:,y1:y2+1,x1:x2+1]
    ...
    new_mask1 = torch.zeros([target.shape[d] for d in chosen_dimensions])
    if channels_dim is not None:
        del indices_to_blend1[channels_dim]
    new_mask1[tuple(indices_to_blend1)] = mask[y1:y2+1,x1:x2+1]

    return blend(target, new_source1, new_mask1,  torch.tensor([0] * len(chosen_dimensions)), mix_gradients, channels_dim,
                 green_function, integration_mode)
