...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.loss_fMSE = MaskWeightedMSE(1000)
    ...
    def reset_lora(self, model):
        # reset
        for n, module in model.named_children():
            if module.__class__.__name__ == "ResnetBlockLoRA":
                for param in module.lora.parameters():
                    param.requires_grad = True
            elif len(list(module.children())) > 0:
                ## compound module, go inside it
                self.reset_lora(module)
    ...
    def instantiate_first_stage(self, config):
        ...
        # if self.enable_lora:
        #     self.reset_lora(self.first_stage_model.decoder.up)
        ...
        if self.model.conditioning_key is not None and not self.force_null_conditioning:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox", "txt"]:
                    ...
                    if self.cond_input_channels == 4:
                        xc = torch.cat(
                            [
                                xc,
                                torch.nn.functional.interpolate(
                                    mask_original, size=(224, 224)
                                ),
                            ],
                            dim=1,
                        )
    ...
    def differentiable_decode_first_stage(
        ...
        z = 1.0 / self.scale_factor * z
        if self.first_stage_model.enable_decoder_cond_lora:
            return self.first_stage_model.decode(z, hs=skip_connect_hs)
        else:
            return self.first_stage_model.decode(z)
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                temp = self.get_learned_conditioning(c["c_crossattn"][0])
                c["c_crossattn"] = [temp]
    ...
    @torch.no_grad()
    def pred_xstart_from_eps(self, x_t, t, epilson):
        b = x_t.shape[0]
        sqrt_one_minus_alphas = self.sqrt_one_minus_alphas_cumprod
        sqrt_one_minus_at = sqrt_one_minus_alphas[t].view(-1, 1, 1, 1).to(x_t.device)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        global x_pred, x_noisy, model_output
        latent_masked_image = None
        input_img = cond["c_concat"][0][:, 3:6, :, :]
        ...
        if self.stage1:
            ...
            model_output = self.apply_model(model_input, t, cond, is_control=False)
        elif self.channels == 9:
            model_input = torch.concatenate([x_noisy, mask, latent_masked_image], dim=1)
            model_output = self.apply_model(model_input, t, cond, is_control=True)
        ...
        loss = F.mse_loss(
            model_output.float() * mask,
            target.float() * mask,
            reduction="none",
        ).mean(dim=(2, 3), keepdim=True)
        loss_scale = (
            (mask.shape[2] * mask.shape[3]) / mask.sum(dim=(2, 3), keepdim=True)
        ).clamp(1, 5)
