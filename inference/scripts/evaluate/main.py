...
select_folder = "imgs1"
(Path(args.output_dir) / select_folder).mkdir(parents=True, exist_ok=True)
...
def possible_resize(image: Image.Image, resolution):
    if image.size != (resolution, resolution):
        image = TF.resize(image, [resolution, resolution], antialias=True)
    return image


# def possible_resize_full_resolution(image: Image.Image, resolution):
#     if image.size != resolution:
#         image = TF.resize(image, resolution, antialias=True)
#     return image


def full_res_resize(image, size):
    image = TF.resize(image, size[::-1], antialias=True)
    return image
...
def process_fn(comp_path, lock):
    ...
    harm_path = os.path.join(
        args.input_dir,
        comp_path.split("/")[-1].replace(".jpg", ".png"),
        # args.input_dir, comp_path.split("/")[-1]
    )
    ...
    if os.path.exists(ot_file):
        ...
        if args.full_resolution:
            ot_img = full_res_resize(ot_img, list(gt_img.size))
        else:
        ...
        ssim_value = ssim(
            ot_np, gt_np, data_range=ot_np.max() - gt_np.min(), channel_axis=-1
        )
...
ssim: {ssim_value:.4f}
        ...
        with lock:
            ...
            ds2ssim[dataset].append(ssim_value)
...
if __name__ == "__main__":
    ...
    for ds in ds2psnr:
        ...
        output_str += f"[SSIM] of {ds} : {np.sum(ds2ssim[ds]):.3f} / {len(ds2ssim[ds])} = {np.mean(ds2ssim[ds]):.4f}\n"
        output_str += "=" * 30 + "\n"
    ...
    ssim : {ssim_total:.3f} / {count} = {ssim_total / count:.4f}
    \n"""
    ...
    # 输出每个数据集的平均PSNR和MSE
    for ds in ds2psnr:
        print_str += f"[PSRN] of {ds} : {np.sum(ds2psnr[ds]):.3f} / {len(ds2psnr[ds])} = {np.mean(ds2psnr[ds]):.3f}\n"
        print_str += f"[MSE] of {ds} : {np.sum(ds2mse[ds]):.3f} / {len(ds2mse[ds])} = {np.mean(ds2mse[ds]):.3f}\n"
        print_str += f"[FMSE] of {ds} : {np.sum(ds2fmse[ds]):.3f} / {len(ds2fmse[ds])} = {np.mean(ds2fmse[ds]):.3f}\n"
        print_str += f"[BMSE] of {ds} : {np.sum(ds2bmse[ds]):.3f} / {len(ds2bmse[ds])} = {np.mean(ds2bmse[ds]):.3f}\n"
        print_str += f"[SSIM] of {ds} : {np.sum(ds2ssim[ds]):.3f} / {len(ds2ssim[ds])} = {np.mean(ds2ssim[ds]):.4f}\n"
        print_str += "=" * 30 + "\n"

    # 输出总体平均PSNR，MSE和FMSE
    print_str += f"""metric in total: 
    total smaples {count}
    psnr : {psnr_total:.3f} / {count} = {psnr_total / count:.3f}
    mse : {mse_total:.3f} / {count} = {mse_total / count:.3f}
    fmse : {fmse_total:.3f} / {count} = {fmse_total / count:.3f}
