...
from src.models.ImgEncoder import FrozenOpenCLIPImageEncoder
...
def main(args):
    ...
    # img_encoder = FrozenOpenCLIPImageEncoder(proj_weight=args.pretrained_cond_model_proj_name_or_path)
    ...
    for step, batch in enumerate(dataloader):
        ...
        # eval_composite_images = batch[args.resolution]["real"]

        generator = torch.Generator(device=accelerator.device).manual_seed(args.seed)
        with torch.inference_mode():
            samples = pipeline(
                image_embeds = batch[args.resolution]['ref'],
            ...
            # print(eval_composite_images.shape)
            # print(samples.shape)
