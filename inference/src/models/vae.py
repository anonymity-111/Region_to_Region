...
from src.models.unet_2d_blocks import (
...
class EncoderSkip(nn.Module):
    ...
    def forward(self, sample: torch.Tensor) -> Tuple[torch.FloatTensor, Tuple[torch.FloatTensor, ...]]:
...
class DecoderSkip(nn.Module):
    ...
    def __init__(
        ...
        # self.mid_skip_conv = zero_module(nn.Conv2d(block_out_channels[-1],block_out_channels[-1],kernel_size=1))
    ...
    def forward(
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            ...
            # # up
            # for up_block in self.up_blocks:
            #     sample, hs = up_block(sample, latent_embeds,hs)


            # middle
            if mid_block_res_samples is not None:
                sample = sample + self.mid_skip_conv(mid_block_res_samples)
            sample = self.mid_block(sample, latent_embeds)
            sample = sample.to(upscale_dtype)
