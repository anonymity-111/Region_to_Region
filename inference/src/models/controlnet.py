...
from .unet_2d_blocks import MACA
...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        channel_size = [320, 640, 1280, 1280]

        # channel_size=[320]*4 + [640]*3 + [1280]*5
    ...
    def forward(
        ...
        for idx, downsample_block in enumerate(self.down_blocks):
            ...
            if hasattr(self, "PCCAs"):
                t_mask = torch.nn.functional.interpolate(
                    mask, size=(sample.shape[-2], sample.shape[-1])
                )
                sample = self.PCCAs[pcca_id](sample, t_mask)
                pcca_id += 1
        ...
        for down_block_res_sample, controlnet_block in zip(
            ...
            # if pcca_id<12:
            #     t_mask = torch.nn.functional.interpolate(
            #         mask, size=(down_block_res_sample.shape[-2], down_block_res_sample.shape[-1])
            #     )
            #     down_block_res_sample = self.PCCAs[pcca_id](down_block_res_sample, t_mask)
            #     pcca_id += 1
