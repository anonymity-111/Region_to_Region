...
class SkipConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, dropout_p=0.1, r=4, scale=1.):
        super().__init__()
...
class ResnetBlockSkip(nn.Module):
    r"""
    A Resnet block.
    ...
    def __init__(
        ...
        *,
        ...
        # if use_skip:
        self.skip_connect = SkipConv2d(in_channels,out_channels)
    ...
    def forward(self, input_tensor: torch.Tensor, temb: torch.Tensor,skip_x = None, *args, **kwargs) -> torch.Tensor:
        if len(args) > 0 or kwargs.get("scale", None) is not None:
            deprecation_message = "The `scale` argument is deprecated and will be ignored. Please remove it, as passing it will raise an error in the future. `scale` should directly be passed while calling the underlying pipeline component i.e., via `cross_attention_kwargs`."
            deprecate("scale", "1.0.0", deprecation_message)
        ...
        hidden_states = self.conv1(hidden_states) if skip_x is None \
                                                    else self.conv1(hidden_states)+self.skip_connect(hidden_states,skip_x)#
