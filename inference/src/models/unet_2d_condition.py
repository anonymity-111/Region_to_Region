...
class UNet2DConditionModel(
    ...
    def __init__(
        ...
        #     [AdCCA(channel) for channel in channel_size]
        # )
    ...
    def forward(
        ...
        if hasattr(self, "skip_connect"):
            ...
            for idx, down_block_res_sample in enumerate(down_block_res_samples):
                t_mask = torch.nn.functional.interpolate(
                    mask,
                    size=(
                        down_block_res_sample.shape[-2],
                        down_block_res_sample.shape[-1],
                    ),
                )

                down_block_res_sample = self.skip_connect[idx](
                    down_block_res_sample, t_mask
                )
                new_down_block_res_samples = new_down_block_res_samples + (
                    down_block_res_sample,
                )
