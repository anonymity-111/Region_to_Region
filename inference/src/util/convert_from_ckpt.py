...
def assign_to_checkpoint(
    ...
    for path in paths:
        ...
        if "lora" in new_path:
            new_path = new_path.replace("lora","skip_connect")
...
def convert_ldm_unet_checkpoint(
    ...
    #
    # while pcca_index < 12:
    #     new_checkpoint[f'skip_connect.{pcca_index}.mlp.0.weight'] = unet_state_dict.pop(f'skip_connect.{pcca_index}.mlp.0.weight')
    #     new_checkpoint[f'skip_connect.{pcca_index}.mlp.2.weight'] = unet_state_dict.pop(f'skip_connect.{pcca_index}.mlp.2.weight')
    #     new_checkpoint[f'skip_connect.{pcca_index}.mlp.0.bias'] = unet_state_dict.pop(f'skip_connect.{pcca_index}.mlp.0.bias')
    #     new_checkpoint[f'skip_connect.{pcca_index}.mlp.2.bias'] = unet_state_dict.pop(f'skip_connect.{pcca_index}.mlp.2.bias')
    #
    #     # new_checkpoint[f'PCCAs.{pcca_index}.mlp1.weight'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp1.weight')
    #     # new_checkpoint[f'PCCAs.{pcca_index}.mlp2.weight'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp2.weight')
    #     # new_checkpoint[f'PCCAs.{pcca_index}.mlp1.bias'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp1.bias')
    #     # new_checkpoint[f'PCCAs.{pcca_index}.mlp2.bias'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp2.bias')
    #     #
    #     # new_checkpoint[f'PCCAs.{pcca_index}.conv1.weight'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.conv1.weight')
    #     # new_checkpoint[f'PCCAs.{pcca_index}.conv2.weight'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.conv2.weight')
    #
    #     pcca_index += 1
    ...
    if controlnet:
        ...
        while pcca_index < 4:
            ...
            new_checkpoint[f'PCCAs.{pcca_index}.mlp.2.weight'] = pop_key_if_exists(unet_state_dict,
                                                                                   f'PCCAs.{pcca_index}.mlp.2.weight')
            new_checkpoint[f'PCCAs.{pcca_index}.mlp.0.bias'] = pop_key_if_exists(unet_state_dict,
                                                                                 f'PCCAs.{pcca_index}.mlp.0.bias')
            new_checkpoint[f'PCCAs.{pcca_index}.mlp.2.bias'] = pop_key_if_exists(unet_state_dict,
                                                                                 f'PCCAs.{pcca_index}.mlp.2.bias')

            # new_checkpoint[f'PCCAs.{pcca_index}.mlp1.weight'] = pop_key_if_exists(unet_state_dict,
            #                                                                       f'PCCAs.{pcca_index}.mlp1.weight')
            # new_checkpoint[f'PCCAs.{pcca_index}.mlp2.weight'] = pop_key_if_exists(unet_state_dict,
            #                                                                       f'PCCAs.{pcca_index}.mlp2.weight')

            # new_checkpoint[f'PCCAs.{pcca_index}.mlp1.bias'] = pop_key_if_exists(unet_state_dict,
            #                                                                     f'PCCAs.{pcca_index}.mlp1.bias')
            # new_checkpoint[f'PCCAs.{pcca_index}.mlp2.bias'] = pop_key_if_exists(unet_state_dict,
            #                                                                     f'PCCAs.{pcca_index}.mlp2.bias')

            new_checkpoint[f'PCCAs.{pcca_index}.conv1.weight'] = pop_key_if_exists(unet_state_dict,
                                                                                   f'PCCAs.{pcca_index}.conv1.weight')
            new_checkpoint[f'PCCAs.{pcca_index}.conv2.weight'] = pop_key_if_exists(unet_state_dict,
                                                                                   f'PCCAs.{pcca_index}.conv2.weight')

            # new_checkpoint[f'PCCAs.{pcca_index}.attention.1.weight'] = pop_key_if_exists(unet_state_dict,
            #                                                                        f'PCCAs.{pcca_index}.attention.1.weight')
            # new_checkpoint[f'PCCAs.{pcca_index}.attention.3.weight'] = pop_key_if_exists(unet_state_dict,
            #                                                                        f'PCCAs.{pcca_index}.attention.3.weight')

            # new_checkpoint[f'PCCAs.{pcca_index}.attention.1.bias'] = pop_key_if_exists(unet_state_dict,
            #                                                                        f'PCCAs.{pcca_index}.attention.1.bias')
            # new_checkpoint[f'PCCAs.{pcca_index}.attention.3.bias'] = pop_key_if_exists(unet_state_dict,
            #                                                                        f'PCCAs.{pcca_index}.attention.3.bias')


        #     new_checkpoint[f'PCCAs.{pcca_index}.mlp.0.weight'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp.0.weight')
        #     # new_checkpoint[f'PCCAs.{pcca_index}.mlp.2.weight'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp.2.weight')
        #     new_checkpoint[f'PCCAs.{pcca_index}.mlp.0.bias'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp.0.bias')
        #     # new_checkpoint[f'PCCAs.{pcca_index}.mlp.2.bias'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp.2.bias')
        #
        #
        # #
        #     # new_checkpoint[f'PCCAs.{pcca_index}.mlp1.weight'] = unet_state_dict.pop(f'PCCAs.{pcca_index}.mlp1.weight')
...
def convert_ldm_vae_checkpoint(checkpoint, config):
    ...
    while filters_index < 12:

        new_checkpoint[f"filters.{filters_index}.Sobel_x"] = vae_state_dict[f"filters.{filters_index}.Sobel_x"]
        new_checkpoint[f"filters.{filters_index}.Sobel_y"] = vae_state_dict[f"filters.{filters_index}.Sobel_y"]
        new_checkpoint[f"filters.{filters_index}.Laplation"] = vae_state_dict[f"filters.{filters_index}.Laplation"]
        new_checkpoint[f"filters.{filters_index}.Edge"] = vae_state_dict[f"filters.{filters_index}.Edge"]
        new_checkpoint[f"filters.{filters_index}.Roberts_x"] = vae_state_dict[f"filters.{filters_index}.Roberts_x"]
        new_checkpoint[f"filters.{filters_index}.Roberts_y"] = vae_state_dict[f"filters.{filters_index}.Roberts_y"]
        new_checkpoint[f"filters.{filters_index}.Sobel_xy"] = vae_state_dict[f"filters.{filters_index}.Sobel_xy"]
        new_checkpoint[f"filters.{filters_index}.Sobel_yx"] = vae_state_dict[f"filters.{filters_index}.Sobel_yx"]
        new_checkpoint[f"filters.{filters_index}.alpha"] = vae_state_dict[f"filters.{filters_index}.alpha"]
        new_checkpoint[f"filters.{filters_index}.beta"] = vae_state_dict[f"filters.{filters_index}.beta"]
    ...
    # # skip_conv
    #
    # new_checkpoint[f"skip_conv.weight"] = vae_state_dict[f"skip_conv.weight"]
    # new_checkpoint[f"skip_conv.bias"] = vae_state_dict[f"skip_conv.bias"]
    #
    # # mid_skip_conv
    # new_checkpoint[f"decoder.mid_skip_conv.weight"] = vae_state_dict[f"decoder.mid_skip_conv.weight"]
    # new_checkpoint[f"decoder.mid_skip_conv.bias"] = vae_state_dict[f"decoder.mid_skip_conv.bias"]
...
def convert_controlnet_checkpoint(
    ...
    return controlnet,converted_ctrl_checkpoint
